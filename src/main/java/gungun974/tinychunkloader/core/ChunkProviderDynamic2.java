...
public class ChunkProviderDynamic2 implements IChunkProvider
{
	...
	public ChunkProviderDynamic2(World world, IChunkLoader chunkLoader, ChunkGenerator chunkGenerator)
	{
	...
	private void saveChunk(Chunk chunk)
	{
		...
		{
			LOGGER.error("Failed to save chunk at {}, {}!", chunk.xPosition, chunk.zPosition, ioexception);
		}
	}
	...
	private Chunk loadChunk(int x, int y)
	{
		...
		try
		{
			Chunk chunk = chunkLoader.loadChunk(world, x, y);
			if(chunk != null)
			{
				chunk.lastSaveTime = world.getWorldTime();
			}
	...
	public boolean tick() {
		...
		for (Chunk chunk : chunkMap.values()) {
			...
			isChunkLoaded(chunk.xPosition,chunk.zPosition); //droppedChunksSet gets updated as side-effect
		}
	...
	public List<Chunk> getForceLoadedChunks() {
		return Collections.unmodifiableList(forceLoadedChunkList);
	}
	...
	public void unloadAllChunks() {
		...
		forceLoadedChunkMap.clear();
		forceLoadedChunkList.clear();
		chunkGenerator = null;
		System.gc();
	}
